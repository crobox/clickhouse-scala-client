...
object ClickhouseHostHealth extends ClickhouseResponseParser {
  ...
  /**
    ...
    * We also set the connection idle timeout to `health-check.timeout` to ensure that the pool will be blocked with on hanging request.
    * */
  def healthFlow(host: Uri)(
    ...
    val healthCachedPool = Http(system).cachedHostConnectionPool[Int](
      ...
      settings = settings
        ...
        .withConnectionSettings(settings.connectionSettings.withIdleTimeout(healthCheckTimeout))
    )
    Source
