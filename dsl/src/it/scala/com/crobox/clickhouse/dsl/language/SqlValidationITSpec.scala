...
class SqlValidationITSpec extends DslITSpec {
  ...
  private val higherOrderConstructs = {
    ...
    Seq(
      ...
      sem("ArraySplit", select(arraySplit[Int, Int]((x, y) => y.notEq(0), Iterable(1, 2, 3), Iterable(1, 0, 1)))),
      sem(
        "ArrayReverseSplit",
        select(arrayReverseSplit[Int, Int]((x, y) => y.notEq(0), Iterable(1, 2, 3), Iterable(1, 0, 1)))
      )
    )
  }
