...
//      (JoinQuery.CrossJoin, 0),
    ...
    it should s"join correctly on: $joinType" in {
      val query: OperationalQuery = joinType match {
        case CrossJoin => select(itemId).from(select(itemId).from(TwoTestTable).join(joinType, ThreeTestTable))
        case _         => select(itemId).from(select(itemId).from(TwoTestTable).join(joinType, ThreeTestTable).using(itemId))
      }
