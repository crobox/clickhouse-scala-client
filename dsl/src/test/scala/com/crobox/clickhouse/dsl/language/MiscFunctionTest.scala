...
class MiscFunctionTest extends ColumnFunctionTest {
  "Tokenization" should "succeed for MiscFunctions" in {
    ...
    r(visibleWidth("1": String)) shouldBe "1"
    r(toTypeName(toUInt64(1: Int))) shouldBe "UInt64"
    r(blockSize()) shouldBe "1"
    r(materialize(1: Int)) shouldBe "1"
    ...
    r(least(3: Int,2 :Int)) shouldBe "2"
    r(greatest(3: Int,2: Int)) shouldBe "3"
    r(uptime()).length should be > 0
    r(version()).length should be > 4
    r(rowNumberInAllBlocks()) shouldBe "0"
    r(runningDifference(1: Int)) shouldBe "0"
    r(mACNumToString(toUInt64(123: Int))) shouldBe "00:00:00:00:00:7B"
