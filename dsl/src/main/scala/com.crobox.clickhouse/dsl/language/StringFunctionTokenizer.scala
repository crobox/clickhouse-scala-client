  ...
  protected def tokenizeStringCol(col: StringFunctionCol[_]): String =
    col match {
      ...
      case Concat(c, c2, cn@_*) =>
        s"concat(${tokenizeColumn(c.column)}, ${tokenizeColumn(c2.column)}${tokenizeSeqCol(cn.map(_.column))})"
      ...
      case SubstringUTF8(c, offset, length) => s"substringUTF8(${tokenizeColumn(c.column)},${tokenizeColumn(offset.column)},${tokenizeColumn(length.column)})"
      case AppendTrailingCharIfAbsent(c, char) => s"appendTrailingCharIfAbsent(${tokenizeColumn(c.column)},${tokenizeColumn(char.column)})"
      case ConvertCharset(c, from, to) => s"convertCharset(${tokenizeColumn(c.column)},${tokenizeColumn(from.column)},${tokenizeColumn(to.column)})"
    }
}
