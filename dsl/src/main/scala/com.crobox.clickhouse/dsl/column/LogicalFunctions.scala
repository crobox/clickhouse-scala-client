...
object LogicalFunctions {
  ...
  implicit class LogicalOperatorImpr(left: LogicalOpsMagnet) {

    def and(right: Option[LogicalOpsMagnet]): LogicalOpsMagnet = right match {
      case Some(r) => LogicalFunction(left, And, r)
      case _       => left
    }

    def and(right: LogicalOpsMagnet): LogicalOpsMagnet = LogicalFunction(left, And, right)

    def or(right: Option[LogicalOpsMagnet]): LogicalOpsMagnet = right match {
      case Some(r) => LogicalFunction(left, Or, r)
      case _       => left
    }

    def or(right: LogicalOpsMagnet): LogicalOpsMagnet = LogicalFunction(left, Or, right)

    def xor(right: Option[LogicalOpsMagnet]): LogicalOpsMagnet = right match {
      case Some(r) => LogicalFunction(left, Xor, r)
      case _       => left
    }

    def xor(right: LogicalOpsMagnet): LogicalOpsMagnet = LogicalFunction(left, Xor, right)
  }

  implicit class OptionalLogicalOperatorImpr(left: Option[LogicalOpsMagnet]) {

    def and(right: Option[LogicalOpsMagnet]): Option[LogicalOpsMagnet] = (left, right) match {
    ...
    def and(right: LogicalOpsMagnet): LogicalOpsMagnet = left match {
      case Some(l) => LogicalFunction(l, And, right)
      case _       => right
    }

    def or(right: Option[LogicalOpsMagnet]): Option[LogicalOpsMagnet] = (left, right) match {
    ...
    def or(right: LogicalOpsMagnet): LogicalOpsMagnet = left match {
      case Some(l) => LogicalFunction(l, Or, right)
      case _       => right
    }

    def xor(right: Option[LogicalOpsMagnet]): Option[LogicalOpsMagnet] = (left, right) match {
    ...
    def xor(right: LogicalOpsMagnet): LogicalOpsMagnet = left match {
